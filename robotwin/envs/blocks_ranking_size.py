    ...
    def load_actors(self):
        ...
        while True:
            ...
            for i in range(3):
                ...
                def check_block_pose(candidate):
                    for existing in block_pose_lst:
                        if np.sum((candidate.p[:2] - existing.p[:2]) ** 2) < 0.01:
                            return False
                    return True
            ...
            eps = [0.12, 0.03]
            block1_pose, block2_pose, block3_pose = [pose.p for pose in block_pose_lst]
            already_ranked = (
                np.all(abs(block1_pose[:2] - block2_pose[:2]) < eps)
                and np.all(abs(block2_pose[:2] - block3_pose[:2]) < eps)
                and block1_pose[0] < block2_pose[0] < block3_pose[0]
            )
        ...
        self.block1_target_pose = [np.random.uniform(-0.1, -0.09), y_pose, 0.74 + self.table_z_bias] + [0, 1, 0, 0]
        self.block2_target_pose = [np.random.uniform(0.01, 0.02), y_pose, 0.74 + self.table_z_bias] + [0, 1, 0, 0]
        self.block3_target_pose = [np.random.uniform(0.08, 0.09), y_pose, 0.74 + self.table_z_bias] + [0, 1, 0, 0]

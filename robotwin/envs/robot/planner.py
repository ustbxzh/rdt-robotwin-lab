...
try:
    if os.environ.get("ROBOTWIN_DISABLE_CUROBO") == "1":
        raise ImportError("Curobo disabled by ROBOTWIN_DISABLE_CUROBO")
    ...
    class CuroboPlanner:
        """Placeholder that keeps MPLib-only environments importable."""

        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "CuroboPlanner is unavailable. Install a compatible Curobo build "
                "or use an MPLib embodiment configuration."
            ) from _CUROBO_IMPORT_ERROR

    if os.environ.get("ROBOTWIN_DISABLE_CUROBO") != "1":
        print('[planner.py]: CuroboPlanner is unavailable; MPLib remains usable.')

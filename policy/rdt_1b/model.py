"""RDT-1B XPolicyLab adapter.

Derived from XPolicyLab (Apache-2.0) and modified by rdt-robotwin-lab for the
curated top-level RDT package and local audit hooks.
"""
...
_REPO_ROOT = _CUR_DIR.parents[1]
_RDT_ROOT = _REPO_ROOT / "rdt"
_CHECKPOINTS_DIR = _CUR_DIR / "checkpoints"

for _path in (str(_REPO_ROOT), str(_CUR_DIR), str(_RDT_ROOT)):
    if _path not in sys.path:
        sys.path.insert(0, _path)

from policy.xpolicylab.model_template import ModelTemplate
from policy.xpolicylab.utils.checkpoint_resolver import resolve_checkpoint_root
from policy.xpolicylab.utils.process_data import (
    decode_image_bit,
    get_robot_action_dim_info,
    unpack_robot_state,
)

from .robodojo_model import create_model
...
class Model(ModelTemplate):
    ...
    def _build_model_args(self):
        ...
        print(
            "[RDT LOAD AUDIT] checkpoint_path =",
    ...
    def update_obs_batch(self, obs_list):
        ...
        if os.environ.get("RDT_INPUT_AUDIT") == "1":
            x = self._latest_encoded_obs_list[0]
            print("\n========== RDT INPUT AUDIT ==========")
            print("prompt:", repr(x["prompt"]))
            print("state shape:", np.asarray(x["state"]).shape)
            print("state:", np.asarray(x["state"]))
            print("cam_high:", np.asarray(x["images"]["cam_high"]).shape)
            print("cam_right_wrist:", np.asarray(x["images"]["cam_right_wrist"]).shape)
            print("cam_left_wrist:", np.asarray(x["images"]["cam_left_wrist"]).shape)
            print("ctrl_freq:", self.args["ctrl_freq"])
            print("=====================================\n")

#!/usr/bin/env bash
# Derived from XPolicyLab (Apache-2.0); modified by rdt-robotwin-lab.
set -euo pipefail
...
echo "[RDT_1B] RDT_DATASET_STAT_PATH=${RDT_DATASET_STAT_PATH}"
...
deepspeed ${RDT_DEEPSPEED_ARGS} main.py \
    ...
    --gradient_accumulation_steps="${RDT_GRADIENT_ACCUMULATION_STEPS:-1}" \

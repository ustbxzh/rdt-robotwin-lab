"""Minimal XPolicyLab policy server.

XPolicyLab Apache-2.0 source, modified by rdt-robotwin-lab for the curated
...
import argparse
import traceback
from policy.xpolicylab.client_server.tcp.model_server import ModelServer
...
def main(deploy_cfg):
    ...
    # Instantiate model
    model_class_func = eval_function_decorator(f"policy.{policy_name}.model", "Model")
    model = model_class_func(deploy_cfg)

    if protocol == "ws":
        try:
            from policy.xpolicylab.client_server.ws.model_server import PolicyServer, PolicyServerConfig

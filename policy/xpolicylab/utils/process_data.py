"""XPolicyLab state/image conversion helpers.

XPolicyLab Apache-2.0 source, modified by rdt-robotwin-lab to resolve the
curated RoboTwin configuration tree.
"""
...
from policy.xpolicylab.utils.load_file import load_json, load_yaml

_PROJECT_ROOT = Path(__file__).resolve().parents[3]
_ENV_CFG_ROOT = Path(
    os.environ.get("ROBOTWIN_ENV_CFG_ROOT", _PROJECT_ROOT / "robotwin" / "env_cfg")
)
...
def get_robot_action_dim_info(env_cfg_type):
    env_cfg = load_yaml(_ENV_CFG_ROOT / f"{env_cfg_type}.yml")
    robot_name = env_cfg['config']['robot']
    robot_action_dim_info = load_json(_ENV_CFG_ROOT / "robot" / "_robot_info.json")[robot_name]
...
def get_batch_size(env_cfg_type):
    ...
    sim_info = load_yaml(_ENV_CFG_ROOT / "sim" / f"{sim_cfg}.yml")
